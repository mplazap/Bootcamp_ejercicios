...
# Filtros globales para todos los graficos
# Es global porque guardamos los filtros en un DF y tabajamos con 
# el en todos los graficos
...
selected_island = st.multiselect('Selecciona una o varias Islas', options=islands, default=islands)

# Filtro por genero
genres = df['sex'].unique().tolist()
selected_genres = st.multiselect('Selecciona uno o los 2 generos', options=genres, default=genres)

# Slider para elegir minimo y maximo de tamaño de aleta
...
# Mostrar datos del Dataframe despues de aplicar los filtros
...
# Boxplot
fig, ax = plt.subplots(figsize=(6,4))
ax.boxplot(x=df_filtered['bill_length_mm'], showmeans=True)
...
# Scatterplot
fig, ax = plt.subplots(figsize=(6, 4))
sns.scatterplot(df_filtered, x='bill_length_mm', y='body_mass_g', hue='species', palette='deep', ax=ax)
ax.set_title('Scatterplot - Relación entre bill_length_mm y body_mass_g')
ax.set_xlabel('bill_length_mm')
ax.set_ylabel('body_mass_g')
ax.legend()
